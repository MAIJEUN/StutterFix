    ...
    internal static class LogExport
    {
        ...
        private static string Report()
        {
            ...
            if (c != null)
            {
                ...
                sb.AppendLine("즉시 이동 최적화 " + On(c.ZeroTween) + ", 즉시 이동 직접 처리 " + On(c.InstantDirect) + ", 투명 장식 빠른 처리 " + On(c.SkipSame) + ", 장식 이동 루프 " + On(c.FastLoop) + ", 미리 확인 " + On(c.Precheck) + ", 장식 위치 계산 줄이기 " + On(c.MoveFinish)

    ...
    internal static class LogExport
    {
        ...
        private static string Report()
        {
            ...
            if (c != null)
            {
                ...
                sb.AppendLine("저사양: 우선순위 " + On(c.LowPriority) + ", 절전 제한 끄기 " + On(c.LowNoThrottle) + ", 음악 반응 계산 끄기 " + On(c.LowNoFft) + ", 게임 화면 해상도 " + c.LowRenderScale + "%" + (c.LowSharpUpscale ? " 선명하게" : "") + ", 장식 이미지 최대 " + (c.LowImageCap > 0 ? c.LowImageCap + "" : "그대로") + ", (실험) 선명도 보정 " + (c.LowSharpen ? c.LowSharpenValue.ToString("F2") : "꺼짐"));

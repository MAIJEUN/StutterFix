...
| 후보 | 방법 | 결과 |
|---|---|---|
| 그리기 비용 | 해상도 낮춤, 스프라이트 6500개 컬링, 타일 메시 컬링 | 변화 없음 |
| 오브젝트 처리 | 장식 오브젝트 통째로 비활성화 | 변화 없음 |
| 파티클 / 애니메이터 | 씬 스캔 | 20개 / 0개, 무시 가능 |
| 설치된 모드 9개 | 전부 제거 후 플레이 | 동일하게 끊김 |
| 애니메이션(tween) | 매 프레임 PauseAll, 재생중 0개 확인 | 154 -> 159 fps (3%), 무거운 구간 최악 28ms 잔존 |
| GPU | PresentMon | 대기 0ms, 병목 아님 |
| 메모리 / 디스크 | 카운터 측정 | 여유 충분 |
| 프레임 제한 | targetFrameRate 확인 기능 | 사용자 확인 결과 아님 |

### 남은 유일한 후보
맵 이벤트 처리 로직. 타일을 밟는 순간 그 타일의 이벤트를 해석하고 적용하는 작업.
- PACL2 비주얼 모드(플레이 안 함)에서는 같은 구간이 매끄럽다는 점과 일치
- Update/LateUpdate가 아니라 타일 진입 시 호출되는 함수라 현재 프로파일러가 못 잡는다

### 다음에 할 일
1. Assembly-CSharp에서 타일 진입 시 호출되는 이벤트 적용 함수를 찾는다 (ILScan 활용)
2. 그 함수들을 Profiler 대상에 추가해 소요 시간을 잰다

### 환경 측정값 (i5-9400F / RTX 4060 Ti / 3440x1440)
- 정상 구간: 프레임 5.5~6.5ms, 게임 스크립트가 차지하는 비중 8%
- 무거운 구간: 프레임 19.8~40ms, 게임 스크립트 비중은 그대로 8%, 나머지는 엔진 내부

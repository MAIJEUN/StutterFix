...
| 게임 화면 해상도 (10~100%) | 플레이 중 게임 화면(타일, 장식, 배경, 필터)만 작게 그려 늘려 보여 줍니다. HUD·설정 창은 선명합니다. 그래픽카드가 약할수록 효과가 가장 큽니다. 부드럽게 / 선명하게(도트) 늘리기를 고를 수 있습니다. 측정: 50% 에서 GPU 시간 약 절반. 자동 검증: 원래 해상도로 따로 그린 화면과 평균 차이 1~14/255, 어긋남 없음 |

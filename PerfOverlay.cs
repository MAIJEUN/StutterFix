...
namespace StutterFix
{
    ...
    public class PerfOverlay : MonoBehaviour
    {
        ...
        private struct TopFrame { public float Ms, T, Fx, Move, Tw, Upd, Late, TwSet; public int N, TwN; public string Ph; }
        ...
        private string TopText()
        {
            ...
            for (int i = 0; i < TopN; i++)
            {
                ...
                sb.AppendFormat(" [{0:F0}ms @{1:F1}초: 효과 {2}개 {3:F1}(장식 이동 {4:F1}), 애니메이션 갱신 {5:F1}{10}, 장식 갱신 {6:F1}+{7:F1}, 나머지 {8:F1}{9}]",
                    f.Ms, f.T, f.N, f.Fx, f.Move, f.Tw, f.Upd, f.Late, f.Ms - f.Fx - f.Tw - f.Upd - f.Late, string.IsNullOrEmpty(f.Ph) ? "" : " / 엔진 단계: " + f.Ph, f.TwN > 0 ? string.Format("(그중 장식 설정 함수 {0}번 {1:F1})", f.TwN, f.TwSet) : "");
            }
            return sb.ToString();
        }
